...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [Tooltip("Dash duration of the character in m/s")]
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                // Jump
                if (_input.jump && _jumpTimeoutDelta <= 0.0f && !_dashing)
                {
        ...
        private void Dash()
        {
            ...
            {
                // Dash finished
                if (_dashTimeout < 0.0f)
                {
                    // Reset dash timeout value.
                    _dashTimeout = DashDuration;
                    _dashing = false;
                    return;
                }
                ...
                {
                    // Dash time running.
                    _dashTimeout -= Time.deltaTime;

                    Vector2 dashDirection = Vector2.up;

                    if (_input.move != Vector2.zero)
                    {
                        dashDirection = _input.move;
                    }

                    // normalise input direction
                    Vector3 inputDirection = new Vector3(dashDirection.x, 0.0f, dashDirection.y).normalized;
                    ...
                    _dashSpeed = Mathf.Round(_dashSpeed * 1000f) / 1000f;

                    // Dash
                    _controller.Move(targetDirection.normalized * (_dashSpeed * Time.deltaTime));
                }
            }
